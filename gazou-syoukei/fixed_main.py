...
class ImageToCharacterApp:
    ...
    def _update_ui_after_processing(self):
        ...
        if self.output_image:
            ...
            messagebox.showerror("エラー", "象形文字の生成に失敗しました。別の画像を試してください。")
    ...
    def generate_character_from_image(self, image_path):
        try:
            ...
            # 輪郭検出（パラメータを調整）
            print("輪郭検出を実行中...")
            contours, _ = cv2.findContours(edges, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)  # RETR_EXTERNALからRERT_LISTに変更
            
            print(f"検出された輪郭の数: {len(contours)}")
            
            # 輪郭がない場合は別の方法を試す
            if not contours:
                print("輪郭が検出されませんでした。別の方法を試します...")
                # 閾値処理を試す
                _, thresh = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
                contours, _ = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
                print(f"閾値処理後の輪郭の数: {len(contours)}")
            ...
            if contours:
                ...
                if filtered_contours:
                    ...
                    print(f"メイン輪郭の面積: {cv2.contourArea(main_contour)}")
                    
                    # 輪郭を単純化
                    epsilon = 0.01 * cv2.arcLength(main_contour, True)
                    approx_contour = cv2.approxPolyDP(main_contour, epsilon, True)
                    print(f"単純化後の輪郭のポイント数: {len(approx_contour)}")
                    ...
                    print("象形文字の生成が完了しました")
                    return character_img, description
                else:
                    print("有効な輪郭が見つかりませんでした")
            
            # 輪郭が見つからない場合は元の画像をグレースケールで返す
            print("輪郭が見つからないため、元の画像を処理します")
            # 画像を二値化して象形文字風に
    ...
    def generate_character_description(self, contour_features, image_name):
        try:
            ...
            print(f"説明の生成中にエラーが発生しました: {str(e)}")
